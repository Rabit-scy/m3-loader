...
static struct trace trace __attribute__((section(".shared_m3_data")));

#if defined TRACE_EARLY_BUF
char trace_early_buf[TRACE_EARLY_BUF_SIZE] __attribute__((section(".shared_m3_data")));
#endif
...
void trace_init(enum port_e port, char *buf, uint32_t size)
{
	...
	if (port != NO_TRACE_PORT) {
		...
		if (trace_early_buf[0] != 0) {
			if (trace.buf_ovf)
				trace_printf(&trace_early_buf[
						strlen(trace_early_buf) + 1]);
			trace_printf(trace_early_buf);
			trace_early_buf[0] = 0;
		}
#endif
