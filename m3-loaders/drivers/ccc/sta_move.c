...
struct ccc_channel *move_get_channel(void *arg)
{
	if (arg)
		       ...
		       struct ccc_dispatcher *dispatcher)
{
	...
	context->cipher = (req->sym.alg != AES_NONE_ALG);
	...
	if (context->cipher) {
		/* MOVE input is plugged on another channel output. */
		context->nr_chunks = 0;
		return context;
	}

	/* MOVE input is in memory. */
...
static inline struct move_data *get_engine_data(__maybe_unused int id)
{
	return &move_data;
}
