...
static int nand_correct_data(uint8_t *buff, uint32_t err_cnt)
{
	...
	for (i = 1; i <= err_cnt; i++) {
		...
		if (err_pos <= 4095) {
			...
			/* Check the bit value */
			if ((buff[byte_to_change] & BIT(flip[bit_to_invert])) == 0) {
				/* Inversion 0->1 */
				buff[byte_to_change] = buff[byte_to_change]
						 | BIT(flip[bit_to_invert]);
			} else {
				/* Inversion 1->0 */
				buff[byte_to_change] = buff[byte_to_change]
						 & ~BIT(flip[bit_to_invert]);
		...
		uint32_t *buff, uint32_t size, uint32_t *total_byte)
{
	...
	while (cnt < cnt_max && (*total_byte < size)) {
		...
		if (err) {
			...
			err = nand_check_and_correct_errors((uint8_t *)tmpbuff);
			if (err)
				goto end;
		}

		if (size - (*total_byte) < 512) {
			memcpy(buff + (*total_byte >> 2), tmpbuff, size - (*total_byte));
			*total_byte = size;
			goto end;
		} else {
