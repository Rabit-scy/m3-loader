 ...
 */
static void ddr3_pub_configure(void)
{
	uint32_t reg;
 ...
 */
static uint32_t ddr3_pub_has_warnings(void)
{
	int i, n;
	uint32_t reg, wrn = 0;
...
int sta_ddr_pub_do_training(uint32_t mask)
{
	...
	ret = ddr3_pub_has_warnings();
	if (ret)
		TRACE_ERR("%s: DDR PHY training ended with warnings\n", __func__);
