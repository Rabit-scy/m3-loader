 ...
 */
static int uart_set_baud_rate(t_uart *uart, unsigned int baudrate)
{
	...
	temp = 16 * baudrate;
	divisor = UART_CLK_CONFIG / temp;
	remainder = UART_CLK_CONFIG % temp;
			...
			unsigned int stop_bits, unsigned int data_bits)
{
	...
	uart_set_baud_rate(uart, baudrate);
