...
static int sta1x95_ddr3_ctl_enable_all_ports(void)
{
	enable_ddr_port(0);
	enable_ddr_port(1);
	enable_ddr_port(2);
	...
	poll_reg(DDR3_CTRL_BASE + PSTAT, DDR3_ALL_PORT_ENABLED_MASK);
...
int sta_ddr_resume(void)
{
	...
	ret = sta1x95_ddr3_ctl_disable_self_refresh();
	if (ret)
		return ret;
...
#ifdef TEST_SELF_REF
	TRACE_NOTICE("%s: checking DDR...\n", __func__);
	sta_ddr_ctl_test_pattern(1, false, 0xdeadbeef, 4, true, false, 0);
#endif
