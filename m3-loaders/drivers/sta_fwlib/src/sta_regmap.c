...
{
	TRACE_INFO("%s @ 0x%x, val_bits=0x%x\n", __func__, reg, val_bits);
				   ...
				   uint32_t access_rights)
{
	...
	TRACE_INFO("%s @ 0x%x, val_bits=0x%x, msk=0x%x, val=0x%x, access_rights=0x%x\n",
		   __func__, reg, val_bits, mask, val, access_rights);

	mask = mask & access_rights;

	switch (val_bits) {
	case STA_REGMAP_VALBITS_8:
		tmp = read_byte_reg(reg);
		tmp &= ~mask;
		write_byte_reg((uint8_t)(tmp | val), reg);
		break;
	case STA_REGMAP_VALBITS_16:
		tmp = read_short_reg(reg);
		tmp &= ~mask;
		write_short_reg((uint16_t)(tmp | val), reg);
		break;
	case STA_REGMAP_VALBITS_32:
	case STA_REGMAP_VALBITS_64: /* FIXME: really handle 64 bits case */
		tmp = read_reg(reg);
		tmp &= ~mask;
		write_reg(tmp | val, reg);
		break;
	default:
		return STA_REGMAP_INVALID_PARAMS;
	}
