...
#define SENSOR_SIZE	(TVDEC_NBCOL * TVDEC_ACTIVE_LINES * \
 ...
 * @param  is_interlaced(IN)	interlacing enabled
				...
				uint32_t **addr)
{
	...
	uint32_t start_index = is_interlaced ? (is_odd ? 1 : 0) : 0;
	uint32_t line_jump = is_interlaced ? 2 : 1;
...
static int build_sga_main_batch(struct cam_rect *output)
{
	...
	surf->width		= TVDEC_NBCOL;
	surf->height		= TVDEC_NBLINES;
	surf->swap_rb		= 0;
...
void cb_frame_handler(void)
{
	...
	while (nb_lines < TVDEC_ACTIVE_LINES) {
		if (ptr[nb_lines * TVDEC_NBCOL * TVDEC_BYTES_PER_PIXEL] == 0x0)
			break;
		nb_lines++;
	}
