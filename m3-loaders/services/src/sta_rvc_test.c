...
#if defined(CAM_SCALING) && !defined(CAM_FULLSCREEN_RESOLUTION)
#define CUSTOM_OUT_WIDTH	800
#define CUSTOM_OUT_HEIGHT	480
#endif

#if !defined(CAM_SCALING)
#define NATIVE_OUT_WIDTH	TVDEC_NBCOL
...
int rvc_start(void)
{
	return manage_camera(1);
}

int rvc_stop(void)
{
	return manage_camera(0);
}
